...
import { API_URL, PUBLIC_API_CALL, SECURE_API_CALL } from './containers/App/constants';
...
function* apiCall(action, authHeader = {}) {
  const { endpoint, payload, method, onStart, onSuccess, onError, identifier, headers } = action;
  const options = {
    method: method || 'GET',
...
function* parseError(error) {
  ...
  try {
    ...
    parsed = error.response
      ? { status: error.response.status, message: error.message || error.response.statusText }
      : { name: error.name, message: error.message, status: error.status, token: error.token };
  }

  return parsed;
}

const sagas = [publicApiSaga, secureApiSaga];
