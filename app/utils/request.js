...
function checkStatus(response) {
  const { status, message, statusText, code } = response;
  if (status >= 200 && status < 300) {
    return response;
  }

  console.log('Req Err 2', message, statusText, status, code);
  const error = new Error(message || statusText || code || status);
...
function parseError(response) {
  ...
  const error = {
    message: data.detail || data.message || statusText,
